...
def calculate_direction(prev_centroid, curr_centroid):
    ...
    x_diff = curr_centroid[0] - prev_centroid[0]
    return 'Right' if x_diff > 0 else 'Left' if x_diff < 0 else 'Stationary'
...
def track_objects(results, frame):
    ...
    cv2.line(frame, (line_x, 0), (line_x, frame.shape[0]), (0, 255, 255), 2) # Line that serves as entrance/exit
    ...
    for i in range(len(boxes)):
        ...
        for obj_id, obj_info in objects.items():
            ...
            obj_id = id_counter
            id_counter += 1
            objects[obj_id] = {'centroid': centroid, 'cls_name': cls_name, 'directions': [], 'side': 'Right' if centroid[0] > line_x else 'Left', 'state': 'Unknown'}

        if objects[obj_id]['directions'] and objects[obj_id]['directions'][-1] != 'Stationary':
            # A person enters the building
            if objects[obj_id]['directions'][-1] == 'Left' and centroid[0] < line_x and objects[obj_id]['side'] == 'Right' and objects[obj_id]['state'] != 'Inside':
                objects[obj_id]['side'] = 'Left'
            ...
            elif objects[obj_id]['directions'][-1] == 'Right' and centroid[0] > line_x and objects[obj_id]['side'] == 'Left' and objects[obj_id]['state'] != 'Outside':
                objects[obj_id]['side'] = 'Right'
    ...
    if 'directions' in objects[obj_id]:
        ...
        if len(objects[obj_id]['directions']) == num_frames:
            ...
            if 'overall_direction' in objects[obj_id] and objects[obj_id]['overall_direction'] != overall_direction:
                ...
                print(objects[obj_id]['overall_direction'])
